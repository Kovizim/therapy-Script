...
--// Função para enviar mensagens no chat global
local function sendMessage(message)
	...
	if ChatRemote then
		...
		warn("Sistema de chat não encontrado.")
	end
end
...
local function createToggle(label, messages)
	toggles[label] = false
	ChatTab:Checkbox({
		Label = label,
		Callback = function(self, Value)
			...
			if Value then
				...
				coroutines[label] = coroutine.create(function()
					while toggles[label] do
						for _, mensagem in ipairs(messages) do
							if not toggles[label] then break end
							sendMessage(mensagem)
							wait(5)
						end
					end
				end)
				coroutine.resume(coroutines[label])
			else
				-- Interrompe a coroutine
				if coroutines[label] and coroutine.status(coroutines[label]) == "suspended" then
					coroutine.close(coroutines[label])
					coroutines[label] = nil
				end
			end
		end,
	})
...
-- Frases originais
createToggle("Ser de carne", {
	"Apesar de parecer um ser de carne",
	"você não tem ideia do que realmente se passa na agitação e no vazio de minha mente",
	"aquele lugar assombra-me constantemente mas por fora estou tão brilhante",
	"e acho que é isso que provoca a escuridão no meu interior..."
})

createToggle("Crítica mais afiada", {
	"Eu prefiro a crítica mais afiada",
	"de um único homem inteligente",
	"que a aprovação irracional",
	"das massas"
})

createToggle("Expectativa e sofrimento", {
	"A expectativa é",
	"a raiz de",
	"todo sofrimento",
	""
...
createToggle("Três classes de homens", {
...
createToggle("Paixão pela curiosidade", {
...
createToggle("Beleza e mistério", {
	"A beleza do mundo está",
	"nos mistérios que ele esconde",
	"e não nas respostas",
	"que buscamos"
})

createToggle("Crescimento na adversidade", {
	"Não há crescimento",
	"sem adversidade",
	"pois é no fogo que",
	"o aço é forjado"
...
createToggle("O valor do silêncio", {
	"Às vezes",
	"o silêncio fala",
	"mais alto que",
	"qualquer palavra"
...
createToggle("O tempo e as feridas", {
	"O tempo não cura",
	"todas as feridas",
	"mas ensina a",
	"viver com elas"
...
createToggle("Horizontes infinitos", {
